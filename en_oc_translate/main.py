...
from utils.checkpoint_handling import check_checkpoint
...
def main():
    ...
    teacher_config = TeacherTrainingConfig(output_dir=os.path.join(base_config.base_dir, "mistral_occitan_finetuned"))
    ...
    curr_time = strftime("%Y-%m-%d %H:%M:%S", localtime(time()))
    student_config = StudentTrainingConfig(output_dir=os.path.join(base_config.base_dir, "tinyllama_distilled"))
